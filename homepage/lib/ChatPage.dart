...
void main() {
  runApp(const MaterialApp(
    home: ChatPage(userID: 'test_user'),
  ));
}
          ...
          Expanded(
            child: TextField(
              ...
              decoration: InputDecoration(
                hintText: 'Search VHT by username... ',
                  ...
                  itemBuilder: (context, index) {
                    return ListTile(
                      leading: CircleAvatar(
                        ...
                        child: Text('${receiverIDs[index][0]}',style: TextStyle(color: Colors.white),),
                      ),
...
Future<List<String>> _fetchReceiverIDs() async {
  ...
  try {
    ...
    for (QueryDocumentSnapshot doc in querySnapshot.docs) {
      ...
      if (doc.id.startsWith(widget.userID)) {
        ...
        if (data.containsKey('messages') && data['messages'] is List) {
          ...
          for (var message in messages) {
            if (message is Map<String, dynamic> && message['senderID'] == widget.userID && message.containsKey('identifier') && message['identifier'] == 'v') {
          ...
          if (!foundV) {
            for (var message in messages) {
              if (message is Map<String, dynamic> && message['receiverID'] == widget.userID && message.containsKey('identifier') && message['identifier'] == 'x') {
...
class _HealthCenterChatState extends State<HealthCenterChat> {
  ...
  Widget _buildChart(List<RiskLevel> riskLevels) {
    // Sort the riskLevels list in ascending order of timestamp
    riskLevels.sort((a, b) => a.date.compareTo(b.date));

    // Create a list to hold data points for the line graph
    List<charts.Series<RiskLevel, int>> series = [
      charts.Series(
        id: 'RiskLevel',
        data: riskLevels,
        domainFn: (RiskLevel riskLevel, index) => index!, // Use the index directly as the x-value
        measureFn: (RiskLevel riskLevel, _) => _mapRiskLevel(riskLevel.level), // Assign y-axis value (1 to 5)
    ...
    return Padding(
      ...
      child: charts.LineChart(
        ...
        primaryMeasureAxis: const charts.NumericAxisSpec(
          tickProviderSpec: charts.StaticNumericTickProviderSpec( // Define custom y-axis labels
            <charts.TickSpec<num>>[
              charts.TickSpec<num>(1, label: 'Very Low'),
              charts.TickSpec<num>(2, label: 'Low'),
              charts.TickSpec<num>(3, label: 'Moderate'),
              charts.TickSpec<num>(4, label: 'High'),
              charts.TickSpec<num>(5, label: 'Very High'),
            ],
          ),
        ),
        domainAxis: charts.NumericAxisSpec(
          tickProviderSpec: charts.BasicNumericTickProviderSpec(desiredTickCount: riskLevels.length), // Set tick count to match the number of risk levels
        ),
      ),
    );
  }

  int _mapRiskLevel(String level) {
...
class _ChatScreenState extends State<ChatScreen> {
                ...
                children: [
                  StreamBuilder(
                    stream: FirebaseFirestore.instance
                        .collection('ChatMessage')
                        .doc("${widget.userID}_${widget.username}")
                        .snapshots(),
  ...
  void sendMessage() async {
    ...
    if (message.isNotEmpty) {
      var docID1 = "${widget.userID}_${widget.username}"; // Form document ID for sender perspective
      ...
      if (existingChatDoc.exists) {
        ...
        await chatRef.doc(docID1).update({
